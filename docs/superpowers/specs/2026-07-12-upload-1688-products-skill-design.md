...
1. 从用户请求中取得并规范化型号，不猜测相近型号。请求没有型号时，读取最近更新的 `automation/*/task_state.json` 并检查带型号会话标签的Chrome页面；只有唯一匹配时才能续传，否则询问用户型号。
2. 确认项目存在，并读取项目 `README.md` 获取当前命令契约。所有项目命令均以 `D:\Auto-Alibab` 为工作目录执行。
3. 检查Python 3.12及项目依赖。只在依赖确实缺失时运行 `python -m pip install -e ".[dev]"`，安装失败则停止并报告。
...
6. 在连接1688前完成本地预检：Excel必须存在精确型号行；`data/processing`、`data/inbox` 或 `data/draft_saved` 中必须存在对应型号目录；PDF、四张当前型号照片、`1688_payload.json`、`image_analysis.json` 和 `detail_assets.json` 必须齐全且型号一致。
7. 检查四张主图均非空且不超过1688当前上传限制。优先使用该型号已有的优化图片；没有合规副本时停止并报告，不直接上传超限文件。
8. 运行 `python -m app.cli run <MODEL> --root D:\Auto-Alibab`。上传器连接发布页面后，以是否出现预期1688商品发布表单判断登录状态；如果跳转登录页、工作台不可用或表单未出现，将其分类为“需要用户登录/页面异常”，不得把 `doctor` 通过当作已登录。
9. 命令失败时读取 `automation/<MODEL>/task_state.json`：
   - `FAILED`：报告保存的异常类型和页面地址；只重试明确可恢复的网络中断或上传中断。
   - `BLOCKED`：报告质量错误和待处理建议，不绕过检查。
   - 登录失效、页面结构变化、素材缺失、相册不存在或图片超限：停止并给出准确处理项。
10. 状态为 `READY_TO_SAVE` 时，再确认质量错误为零、详情图片数为5、四张主图已就绪，并向用户报告当前页面已停在保存草稿前。
...
Skill 不重复实现商品加载、表单填写、详情生成或质量检测逻辑。若静态文件检查与上传器的真实校验结果冲突，以上传器返回结果为准，并停止等待人工复核。

## 验证

- RED：记录未安装 Skill 时，新会话无法自动获得项目路径、命令、安全边界、无型号续传和失败分类规则的基线缺口。
- GREEN：验证 Skill 元数据、目录结构和内容规范；用不执行真实发布的场景测试型号提取、缺失素材、9223不可用、未登录、失败状态和 `READY_TO_SAVE` 报告。
